...
class List1(Enum):
    ...
    :cvar VALUE_04: Update (partial) In ONIX 3.0 only, use when sending
        a ‘block update’ record. A block update implies using the
        supplied block(s) to update the existing record for the product,
        replacing only the blocks included in the block update, and
        leaving other blocks unchanged – for example, replacing old
        information from Blocks 4 and 6 with the newly-received data
        while retailing information from Blocks 1–3 and 5 untouched. In
        previous ONIX releases, and for ONIX 3.0 using other
        notification types, updating is by replacing the complete record
    ...
    :cvar VALUE_88: Test update (Partial) ONIX 3.0 only. Record may be
        processed for test purposes, but data should be discarded when
        testing is complete. Sender must ensure the
...
class List100(Enum):
    ...
    :cvar VALUE_07: ISNI-based discount group code ISNI-based discount
        ...
        customer. For use in ONIX 3.0 only
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
    VALUE_03 = "03"
...
class List102(Enum):
    ...
    :cvar VALUE_04: Retail sales outlet GLN 13-digit GS1 global location
        number (formerly EAN location number). For use in ONIX 3.0 only
    :cvar VALUE_05: Retail sales outlet SAN 7-digit Book trade Standard
        Address Number (US, UK etc). For use in ONIX 3.0 only
    """
    VALUE_01 = "01"
    VALUE_03 = "03"
    VALUE_04 = "04"
...
class List12(Enum):
    ...
    :cvar VALUE_15: Superpocket book In countries where recognised as a
        distinct trade category, eg Italy «supertascabile». For use in
        ONIX 3.0 only
    :cvar VALUE_16: Beau-livre Category of books, usually hardcover and
        ...
        books’ or ‘art books’ in English. For use in ONIX 3.0 only
    :cvar VALUE_17: Podcast Category of audio products typically
        distinguished by being free of charge (but which may be
        monetised through advertising content) and episodic. For use in
        ONIX 3.0 only
    :cvar VALUE_18: Periodical Category of books or e-books which are
        ...
        products. For use in ONIX 3.0 only
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
    VALUE_03 = "03"
...
class List121(Enum):
    ...
    :cvar ADLM: Adlam For use in ONIX 3.0 only
    :cvar AFAK: Afaka Script is not supported by Unicode
    :cvar AGHB: Caucasian Albanian Ancient/historic script. For use in
        ONIX 3.0 only
    :cvar AHOM: Ahom, Tai Ahom Ancient/historic script. For use in ONIX
        3.0 only
    :cvar ARAB: Arabic
    ...
    :cvar BHKS: Bhaiksuki Ancient/historic script. For use in ONIX 3.0
        only
    ...
    :cvar DOGR: Dogra Script is not supported by Unicode. For use in
        ONIX 3.0 only
    ...
    :cvar GONG: Gunjala Gondi Script is not supported by Unicode. For
        use in ONIX 3.0 only
    :cvar GONM: Masaram Gondi For use in ONIX 3.0 only
    :cvar GOTH: Gothic Ancient/historic script
    :cvar GRAN: Grantha Ancient/historic script
    :cvar GREK: Greek
    :cvar GUJR: Gujarati
    :cvar GURU: Gurmukhi
    :cvar HANB: Han with Bopomofo See Hani, Bopo. For use in ONIX 3.0
        only
    ...
    :cvar HATR: Hatran Ancient/historic script. For use in ONIX 3.0 only
    :cvar HEBR: Hebrew
    :cvar HIRA: Hiragana
    :cvar HLUW: Anatolian Hieroglyphs (Luwian Hieroglyphs, Hittite
        Hieroglyphs) Ancient/historic script. For use in ONIX 3.0 only
    ...
    :cvar KITL: Khitan large script Script is not supported by Unicode.
        For use in ONIX 3.0 only
    :cvar KITS: Khitan small script Script is not supported by Unicode.
        For use in ONIX 3.0 only
    :cvar KNDA: Kannada
    ...
    :cvar LATN: Latin
    :cvar LEKE: Leke Script is not supported by Unicode. For use in ONIX
        3.0 only
    ...
    :cvar MAHJ: Mahajani Ancient/historic script. For use in ONIX 3.0
        only
    :cvar MAKA: Makasar Script is not supported by Unicode. For use in
        ONIX 3.0 only
    :cvar MAND: Mandaic, Mandaean
    :cvar MANI: Manichaean Ancient/historic script
    :cvar MARC: Marchen Ancient/historic script. For use in ONIX 3.0
        only
    ...
    :cvar MEDF: Medefaidrin (Oberi Okaime, Oberi Ɔkaimɛ) Script is not
        supported by Unicode. For use in ONIX 3.0 only
    ...
    :cvar MLYM: Malayalam
    :cvar MODI: Modi, Moḍī Ancient/historic script. For use in ONIX 3.0
        only
    ...
    :cvar MULT: Multani Ancient/historic script. For use in ONIX 3.0
        only
    ...
    :cvar NEWA: Newa, Newar, Newari, Nepāla lipi For use in ONIX 3.0
        only
    ...
    :cvar OSGE: Osage For use in ONIX 3.0 only
    :cvar OSMA: Osmanya
    :cvar PALM: Palmyrene Ancient/historic script
    :cvar PAUC: Pau Cin Hau For use in ONIX 3.0 only
    ...
    :cvar SOYO: Soyombo For use in ONIX 3.0 only
    :cvar SUND: Sundanese
    ...
    :cvar ZANB: Zanabazar Square (Zanabazarin Dörböljin Useg, Xewtee
        Dörböljin Bicig, Horizontal Square Script) For use in ONIX 3.0
        only
    :cvar ZINH: Code for inherited script
    :cvar ZMTH: Mathematical notation Not a script in Unicode
    :cvar ZSYE: Symbols (Emoji variant) Not a script in Unicode. For use
        in ONIX 3.0 only
...
class List141(Enum):
    ...
    :cvar VALUE_02: GTIN-13 Barcode uses 13-digit EAN symbology (without
        5-digit extension)
    :cvar VALUE_03: GTIN-13+5 (US dollar price encoded) EAN symbology,
        first digit of 5-digit extension is 1–5
    :cvar VALUE_04: GTIN-13+5 (CAN dollar price encoded) EAN symbology,
        first digit of 5-digit extension is 6
    :cvar VALUE_05: GTIN-13+5 (no price encoded) EAN symbology, 5-digit
        extension is 90000, or extension does not indicate a price
    ...
    :cvar VALUE_10: GTIN-13+5 (UK Pound Sterling price encoded) EAN
        symbology, first digit of 5-digit extension is 0
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List142(Enum):
    ...
    :cvar VALUE_00: Unknown / unspecified Position unknown or
        unspecified
    :cvar VALUE_01: Cover 4 The back cover of a book
...
class List145(Enum):
    ...
    :cvar VALUE_07: Time-limited license E-publication license is time
        limited. Use with 02 from List 146 and either a time period in
        days, weeks or months, or a Valid until date in
        &lt;EpubUsageLimit&gt;. The purchased copy becomes unusable when
        the license expires
    ...
    :cvar VALUE_09: Multi-user license E-publication license is multi-
        ...
        product should be given in &lt;EpubUsageLimit&gt;
...
class List149(Enum):
    ...
    :cvar VALUE_05: Master brand The title element names a master brand
        where the use of the brand spans multiple collections and
...
class List150(Enum):
    ...
    :cvar AZ: Other audio format Other audio format not specified by AB
        to AM. Further detail is expected in
    ...
    :cvar BF: Pamphlet Pamphlet, stapled; use for German ‘geheftet’.
    ...
    :cvar BO: Fold-out book or chart Concertina-folded booklet or chart,
        ...
        bound within distinct board or card covers: use for German
        ‘Leporello’
    ...
    :cvar DA: Digital (on physical carrier) Digital content delivered on
        a physical carrier (detail unspecified)
    :cvar DB: CD-ROM
    ...
    :cvar ZX: Other toy/game accessories Other toy, game and puzzle
        items not specified by ZB to ZN, generally accessories to other
        products etc. Further detail is expected in
        &lt;ProductFormDescription&gt;, as &lt;ProductFormDetail&gt; and
        &lt;ProductFormFeature&gt; are unlikely to be sufficient
    :cvar ZY: Other apparel Other apparel items not specified by ZB to
        ZN, including branded, promotional or tie-in scarves, caps,
    ...
    :cvar ZZ: Other merchandise Other merchandise not specified by ZB to
        ZY. Further detail is expected in
        &lt;ProductFormDescription&gt;, as &lt;ProductFormDetail&gt; and
        &lt;ProductFormFeature&gt; are unlikely to be sufficient
    """
    VALUE_00 = "00"
...
class List151(Enum):
    ...
    :cvar VALUE_09: Registered in The place of legal registration of an
        organisation
    :cvar VALUE_10: Operating from The place an organisation or part of
        an organisation is based or operates from
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List153(Enum):
    ...
    :cvar VALUE_02: Short description/annotation Limited to a maximum of
        350 characters
    :cvar VALUE_03: Description Length unrestricted
...
class List155(Enum):
    ...
    :cvar VALUE_24: From… until date Combines From date and Until date
        to define a period (both dates are inclusive). Use with for
        example dateformat 06
...
class List158(Enum):
    ...
    :cvar VALUE_36: Online advertisement page German ‘Búhnenbild’
...
class List16(Enum):
    ...
    :cvar VALUE_39: ISCC International Standard Content Code, a
        ‘similarity hash’ identifier derived algorithmically from the
        content itself (see https://iscc.codes). &lt;IDValue&gt; is the
        27-character case-sensitive string (including one hyphen)
        comprising the Meta-ID and Content-ID components of a full ISCC
        generated from a digital manifestation of the work. Use only
        with ONIX 3.0
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List162(Enum):
    ...
    :cvar VALUE_09: ISCC International Standard Content Code, a
        ...
        content itself (see https://iscc.codes). &lt;IDValue&gt; is the
        55-character case-sensitive string (including three hyphens)
...
class List163(Enum):
    ...
    :cvar VALUE_12: Last reprint date Date when a product was last
        reprinted
    :cvar VALUE_13: Out-of-print / deletion date Date when a product was
        (or will be) declared out-of-print or deleted
...
class List164(Enum):
    ...
    :cvar VALUE_01: Manifestation of Product X is or includes a
        manifestation of work Y. (There is a direct parent-child
        relation between work Y and the product)
    :cvar VALUE_02: Derived from Product X is or includes a
        manifestation of a work derived (directly) from related work Y
        in one or more of the ways specified in ISTC rules. (There is a
        relationship between a grandparent work Y and a parent work, and
        between that parent work and the product.) This relation type is
        intended to enable products with a common ‘grandparent’ work to
        be linked without specifying the precise nature of their
        derivation, and without necessarily assigning an identifier to
        the product’s parent
    :cvar VALUE_03: Related work is derived from this Product X is a
        manifestation of a work from which related work Y is (directly)
        derived in one or more of the ways specified in ISTC rules.
        (There is a relationship between a parent work and a child work
        Y, and between the parent work and the product)
    :cvar VALUE_04: Other work in same collection Product X is a
        manifestation of a work in the same collection as related work Y
    :cvar VALUE_05: Other work by same contributor Product X is a
        manifestation of a work by the same contributor(s) as related
        work Y
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List168(Enum):
    ...
    :cvar VALUE_04: Number of copies of this product Use with Price
        ...
        condition qualifies for purchase of the specified number of
        copies of this product at this price. Use for example when
        describing a price that applies to the specified number of units
        of this product which is valid if they are purchased along with
        a number of copies of another product
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
    VALUE_03 = "03"
...
class List17(Enum):
    ...
    :cvar A51: Research by Person or organization responsible for
        performing research on which the work is based. For use in ONIX
        3.0 only
    ...
    :cvar B30: Historical advisor For use in ONIX 3.0 only
    :cvar B31: Original editor Editor of the first edition (usually of a
        standard work) who is not an editor of the current edition. For
        use in ONIX 3.0 only
    :cvar B32: Translation revised by For use in ONIX 3.0 only
    ...
    :cvar D04: Choreographer Of a dance performance. For use in ONIX 3.0
        only
...
class List171(Enum):
    ...
    :cvar VALUE_01: VAT Value-added tax (TVA, IVA, MwSt etc)
    :cvar VALUE_02: GST General sales tax
...
class List175(Enum):
    ...
    :cvar B120: 46-Henkei size Japanese format, 188 x 127mm
    :cvar B121: A4 297 x 210mm
    :cvar B122: A4-Henkei size Japanese format, 297 x 210mm
    :cvar B123: A5-Henkei size Japanese format, 210 x 146mm
    :cvar B124: B5-Henkei size Japanese format, 257 x 182mm
    :cvar B125: B6-Henkei size Japanese format, 182 x 128mm
    ...
    :cvar B132: Libro de bolsillo Spanish pocket paperback. Use with
        Product form code BC
    :cvar B133: Pocket-sized German ,Taschenbuch‘, Italian «Tascabile /
        Supertascabile» pocket-sized format, usually less than about
    ...
    :cvar L101: Laminated Whole product laminated (eg laminated map,
        ...
        laminated cover
    :cvar P101: Desk calendar or diary Large format. Use with Product
        Form code PC or PF
    :cvar P102: Mini calendar or pocket diary Small format. Use with
        Product Form code PC or PF
    :cvar P103: Engagement calendar or Appointment diary Usually with
        time-of-day subdivisions (rather than just days). Use with
        Product Form code PC or PF
    :cvar P104: Day by day calendar Eg tear-off calendars. Use with
    ...
    :cvar P106: Wall calendar Large calendar intended for hanging,
        usually one page per month. Use with Product Form code PC
...
class List18(Enum):
    ...
    :cvar VALUE_07: Fictional character name Use only within
        &lt;NameAsSubject&gt;, to indicate the subject is fictional. For
        use in ONIX 3.0 only
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
...
class List19(Enum):
    ...
    :cvar VALUE_04: Various Use for example when the product is a pack
        of books by different authors
    :cvar VALUE_05: Synthesised voice – male Use with Contributor role
        code E07 ‘read by’, eg for audio books
...
class List198(Enum):
    ...
    :cvar VALUE_08: CIP / Legal deposit contact
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List21(Enum):
    ...
    :cvar HRE: High readability edition Edition optimised for high
        ...
        if the type size is significantly larger than normal. For use in
        ONIX 3.0 only
    ...
    :cvar NUM: Edition with numbered copies A limited edition in which
        each copy is individually numbered, and the actual number of
        copies is strictly limited. Note that the supplier may limit the
        number of orders fulfilled per retail outlet. Use
    ...
    :cvar UNN: Edition with unnumbered copies A limited edition in which
        ...
        number of copies is strictly limited. Note that the supplier may
        limit the number of orders fulfilled per retail outlet. Use
        &lt;EditionStatement&gt; to give details of the number of copies
        printed
    ...
    :cvar VOR: Vorlesebücher Readaloud edition – specifially intended
        and designed for reading aloud (to children). For use in ONIX
        3.0 only
    """
    ABR = "ABR"
    ACT = "ACT"
...
class List22(Enum):
    ...
    :cvar VALUE_12: Language of notes Use for the language of footnotes,
        endnotes, annotations or commentary, etc, where it is different
        from the language of the main text
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
    VALUE_03 = "03"
...
class List23(Enum):
    ...
    :cvar VALUE_17: Number of cards In a pack of educational flash
        cards, playing cards, postcards, greeting cards etc. For use in
        ONIX 3.0 only
    ...
    :cvar VALUE_23: Storage filesize Approximate size of storage space
        ...
        specified extent unit. For use in ONIX 3.0 only
    """
    VALUE_00 = "00"
    VALUE_02 = "02"
    VALUE_03 = "03"
...
class List230(Enum):
    ...
    :cvar VALUE_06: Lend Lendable by the purchaser to other device
        ...
        loan, unless a number of concurrent borrowers is also specified
    :cvar VALUE_07: Time-limited license E-publication license is time-
        limited. Use with code 02 from List 146 and a time period in
        days, weeks or months in &lt;PriceConstraintLimit&gt;. The
    ...
    :cvar VALUE_09: Multi-user license E-publication license is multi-
        user. Maximum number of concurrent users licensed to use the
        product should be given in &lt;PriceConstraintLimit&gt;
    :cvar VALUE_10: Preview on premises Preview locally before purchase.
        Allows a retail customer, account holder or patron to view a
        proportion of the book (or the whole book, if no proportion is
        specified) before purchase, but ONLY while located physically in
...
class List24(Enum):
    ...
    :cvar VALUE_00: Physical pieces For example a count of the
        individual number of cards in a pack. For use in ONIX 3.0 only
    :cvar VALUE_01: Characters Approximate number of characters
        (including spaces) of natural language text. For use in ONIX 3.0
        only
    ...
    :cvar VALUE_12: Discs Of an audiobook on multiple Red Book audio
        ...
        structural breaks in the text (eg chapter breaks). For use in
        ONIX 3.0 only
    ...
    :cvar VALUE_16: Hours minutes seconds HHHMMSS Fill with leading
        ...
        required, use HHHMMSScc (in ONIX 3.0 only)
    :cvar VALUE_17: Bytes
...
class List25(Enum):
    ...
    :cvar VALUE_30: Table of contents For use in ONIX 3.0 only
    :cvar VALUE_31: Gazetteer Geographical index. For use in ONIX 3.0
        only
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
...
class List27(Enum):
    ...
    :cvar VALUE_12: BIC subject category For all BIC subject codes and
        qualifiers, see https://www.bic.org.uk/7/BIC-Standard-Subject-
        Categories/
    ...
    :cvar VALUE_21: BIC children’s book marketing category See PA/BIC
        CBMC guidelines at http://www.bic.org.uk/8/Children%27s-Books-
    ...
    :cvar VALUE_65: Allärs Allmän tesaurus på svenska: Swedish
        ...
        http://onki.fi/fi/browser/ (in Finnish). Deprecated. No longer
    ...
    :cvar B1: BISG Educational Taxonomy Controlled vocabulary for
        ...
        https://bisg.org/page/EducationalTaxonomy
    ...
    :cvar B5: Key place names For fiction and non-fiction, one or more
        ...
        names should be separated by semi-colons. For use in ONIX 3.0
        only
    ...
    :cvar B7: NGSS Next Generation Science Standards for K-12 education
        ...
        is a code such as 4-PS3-2. For use in ONIX 3.0 only
    :cvar B8: MVB-Lesemotive MVB classification of ‘reading rationales’,
        ...
        Categories are assigned and maintained by MVB. For use in ONIX
        3.0 only. See https://vlb.de/lesemotive
    :cvar B9: LOPS21 Subject module Finnish Suomalainen
        oppiaineluokitus. For use in ONIX 3.0 only
    :cvar C0: Læreplaner-LK20 Codes for Norwegian curriculum for primary
        and secondary education. For use in ONIX 3.0 only. See
        Læreplaner-LK20 at https://www.udir.no/om-udir/data/kl06-grep/
    :cvar C1: Kompetansemål-LK20 Codes for competency aims in the
        Norwegian curriculum for primary and secondary education. For
        use in ONIX 3.0 only. See Kompetansemål-LK20 at
    ...
    :cvar C2: Kompetansemålsett-LK20 Codes for sets of competency aims
        ...
        For use in ONIX 3.0 only. See Kompetansemålsett-LK20 at
        https://www.udir.no/om-udir/data/kl06-grep/
    :cvar C3: Tverrfaglige temaer-LK20 Codes for interdisciplinary
        topics in the Norwegian curriculum for primary and secondary
        education. For use in ONIX 3.0 only. See Tverrfaglige temaer-
        LK20 at https://www.udir.no/om-udir/data/kl06-grep/
    :cvar C4: CLIL – Type d’article scolaire For use in ONIX 3.0 only
    :cvar C5: GAR – Type pédagogique Gestionnaire d’Accès aux resources
        – see https://gar.education.fr/ For use in ONIX 3.0 only
    :cvar C6: ISCED-F UNESCO ISCED Fields of education and training
        (2013), eg &lt;SubjectCode&gt; 0222 is ‘History and
        archaeology’. For use in ONIX 3.0 only. See
    ...
    :cvar C7: Klassifikationen von Spielen, Puzzles und Spielwaren
        German category scheme for games, puzzles and toys. For use in
        ONIX 3.0 only. See
        https://www.ludologie.de/fileadmin/user_upload/PDFs/211126_Kategorisierung_von_Spielen_Puzzles_und_Spielwaren.pdf
    :cvar C8: NBVok National Library of Norway genre and form thesaurus.
        For use in ONIX 3.0 only. See https://www.nb.no/nbvok/ntsf
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
    VALUE_03 = "03"
...
class List28(Enum):
    ...
    :cvar VALUE_01: General / adult For a non-specialist adult audience.
        Consider also adding an ONIX Adult audience rating
    ...
    :cvar VALUE_11: Pre-primary education Equivalent to UNESCO’s ISCED
        ...
        audience, code 04). For use in ONIX 3.0 only
    :cvar VALUE_12: Primary education Equivalent to ISCED Level 1. For
        use in ONIX 3.0 only
    :cvar VALUE_13: Lower secondary education Equivalent to ISCED Level
        2 (general and vocational). For use in ONIX 3.0 only
    :cvar VALUE_14: Upper secondary education Equivalent to ISCED Level
        3 (general and vocational). For use in ONIX 3.0 only
    ...
    :cvar VALUE_07: ELT/ESL Intended for use in teaching English as a
        second, non-native or additional language. Indication of the
        language level (eg CEFR) should be included where possible. An
        audience range should also be included if the product is (also)
        suitable for use in primary and secondary education
    :cvar VALUE_09: Second language teaching Intended for use in
        teaching second, non-native or additional languages, for example
        teaching German to Spanish speakers. Indication of the language
        level (eg CEFR) should be included where possible. An audience
        range should also be included if the product is (also) suitable
        for use in primary and secondary education. Prefer code 07 for
...
class List29(Enum):
    ...
    :cvar VALUE_30: ISCED 2011 International Standard Classification of
        ...
        direct access to upper secondary education’. For use in ONIX 3.0
        only. See http://uis.unesco.org/en/topic/international-standard-
...
class List30(Enum):
    ...
    :cvar VALUE_31: Brazil Education level Nível de Educação do Brasil,
        see List 238. For use in ONIX 3.0 only
    :cvar VALUE_32: French educational levels Basic French educational
        level classification. Values are defined by ScoLOMFR. For use in
        ONIX 3.0 only. See
        http://data.education.fr/voc/scolomfr/scolomfr-voc-012
    :cvar VALUE_33: Finnish Upper secondary school course (2021+) For
        use in ONIX 3.0 only
    :cvar VALUE_34: French educational school levels Values are defined
        by ScoLOMFR. For use in ONIX 3.0 only. See
...
class List32(Enum):
    ...
    :cvar VALUE_11: LIX Swedish ‘läsbarhetsindex’ readability index used
        in Scandinavia. For use in ONIX 3.0 only
    :cvar VALUE_12: Lexile Audio measure Lexile Audio measure from
        ...
        of audio material (for example 600L or 1030L). For use in ONIX
        3.0 only. See https://lexile.global/the-lexile-framework-for-
        listening/
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List44(Enum):
    ...
    :cvar VALUE_36: Nasjonalt autoritetsregister Nasjonalt
        ...
        for personal and corporate names. For use in ONIX 3.0 only
    :cvar VALUE_37: GRID Global Research Identifier Database ID (see
        https://www.grid.ac). For use in ONIX 3.0 only
    :cvar VALUE_38: IDRef Party ID from Identifiers and Standards for
        ...
        pour l’enseignement supérieur et la recherche). For use on ONIX
        3.0 only. See https://www.idref.fr
...
class List48(Enum):
    ...
    :cvar VALUE_06: Unit volume The volume of the product, including any
        ...
        millilitres), and code oz = fluid ounces. For use in ONIX 3.0
        only
    :cvar VALUE_07: Unit capacity Volume of the internal (fluid)
        ...
        oz = fluid ounces. For use in ONIX 3.0 only
    :cvar VALUE_08: Unit weight The weight of the product, including any
        retail packaging
    ...
    :cvar VALUE_14: Unpackaged height As height, but of the product
        ...
        code 01). For use in ONIX 3.0 only
    :cvar VALUE_15: Unpackaged width As width, but of the product
        without packaging (use only for products supplied in retail
        packaging, must also supply overall size when packaged using
        code 02). For use in ONIX 3.0 only
    :cvar VALUE_16: Unpackaged thickness As thickness, but of the
        ...
        using code 03). For use in ONIX 3.0 only
    ...
    :cvar VALUE_18: Total weight of Lithium Mass or equivalent mass of
        ...
        be calculated from the number of batteries if required. For use
        in ONIX 3.0 only
    ...
    :cvar VALUE_23: Carton length Includes packaging. See
        ...
        convenient to handle. For use in ONIX 3.0 only
    ...
    :cvar VALUE_26: Carton weight Includes the weight of product(s)
        ...
        (code 08) AND &lt;PackQuantity&gt; are supplied. For use in ONIX
        3.0 only
...
class List49(Enum):
    ...
    :cvar BE_BRU: Brussels-Capital Region For use in ONIX 3.0 only
    :cvar BE_VLG: Flemish Region For use in ONIX 3.0 only
    :cvar BE_WAL: Walloon Region For use in ONIX 3.0 only
    :cvar CA_AB: Alberta
    ...
    :cvar CN_BJ: Beijing Municipality For use in ONIX 3.0 only
    :cvar CN_TJ: Tianjin Municipality For use in ONIX 3.0 only
    :cvar CN_HE: Hebei Province For use in ONIX 3.0 only
    :cvar CN_SX: Shanxi Province For use in ONIX 3.0 only
    :cvar CN_NM: Nei Mongol Autonomous Region For use in ONIX 3.0 only
    :cvar CN_LN: Liaoning Province For use in ONIX 3.0 only
    :cvar CN_JL: Jilin Province For use in ONIX 3.0 only
    :cvar CN_HL: Heilongjiang Province For use in ONIX 3.0 only
    :cvar CN_SH: Shanghai Municipality For use in ONIX 3.0 only
    :cvar CN_JS: Jiangsu Province For use in ONIX 3.0 only
    :cvar CN_ZJ: Zhejiang Province For use in ONIX 3.0 only
    :cvar CN_AH: Anhui Province For use in ONIX 3.0 only
    :cvar CN_FJ: Fujian Province For use in ONIX 3.0 only
    :cvar CN_JX: Jiangxi Province For use in ONIX 3.0 only
    :cvar CN_SD: Shandong Province For use in ONIX 3.0 only
    :cvar CN_HA: Henan Province For use in ONIX 3.0 only
    :cvar CN_HB: Hubei Province For use in ONIX 3.0 only
    :cvar CN_HN: Hunan Province For use in ONIX 3.0 only
    :cvar CN_GD: Guangdong Province For use in ONIX 3.0 only
    :cvar CN_GX: Guangxi Zhuangzu Autonomous Region For use in ONIX 3.0
        only
    :cvar CN_HI: Hainan Province For use in ONIX 3.0 only
    :cvar CN_CQ: Chongqing Municipality For use in ONIX 3.0 only
    :cvar CN_SC: Sichuan Province For use in ONIX 3.0 only
    :cvar CN_GZ: Guizhou Province For use in ONIX 3.0 only
    :cvar CN_YN: Yunnan Province For use in ONIX 3.0 only
    :cvar CN_XZ: Tibet Autonomous Region For use in ONIX 3.0 only
    :cvar CN_SN: Shaanxi Province For use in ONIX 3.0 only
    :cvar CN_GS: Gansu Province For use in ONIX 3.0 only
    :cvar CN_QH: Qinghai Province For use in ONIX 3.0 only
    :cvar CN_NX: Ningxia Huizu Autonomous Region For use in ONIX 3.0
        only
    :cvar CN_XJ: Xinjiang Uygur Autonomous Region For use in ONIX 3.0
        only
    :cvar CN_TW: Taiwan Province Prefer code TW (Taiwan, Province of
        China) from List 91. For use in ONIX 3.0 only
    :cvar CN_HK: Hong Kong Special Administrative Region Prefer code HK
        (Hong Kong) from List 91. For use in ONIX 3.0 only
    :cvar CN_MO: Macau Special Administrative Region Prefer code MO
        (Macao) from List 91. For use in ONIX 3.0 only
...
class List51(Enum):
    ...
    :cvar VALUE_04: Has companion product &lt;Product&gt; and
        ...
        used, or are usable, together (is own inverse). For use in ONIX
        3.0 only
    ...
    :cvar VALUE_43: Adapted from &lt;Product&gt; is an adapted
        ...
        &lt;RelatedProduct&gt; (inverse of code 44). For use in ONIX 3.0
        only
    :cvar VALUE_44: Adapted as &lt;Product&gt; is the original from
        which &lt;RelatedProduct&gt; is adapted (dramatized etc)
        (inverse of code 43), For use in ONIX 3.0 only
    :cvar VALUE_45: Linked product offer Purchases of &lt;Product&gt;
        may qualify for one or more copies of &lt;RelatedProduct&gt;
        either free of charge or at a reduced price, or vice versa (is
        own inverse). This may be dependent on retailer participation,
        upon price and upon the quantity of the &lt;Product&gt; or the
...
class List57(Enum):
    ...
    :cvar VALUE_07: Calculated from contents Price calculated as sum of
        individual prices of components listed as Product parts. For use
        in ONIX 3.0 only
    :cvar VALUE_08: Supplier does not supply The supplier does not
        ...
        but has not set a price for part of the market. For use in ONIX
        3.0 only
    """
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List59(Enum):
    ...
    :cvar VALUE_17: Introductory price Initial ‘Introductory offer’
        ...
        offer expires (Fr. Prix de lancement). Only valid in ONIX 3.0
...
class List62(Enum):
    ...
    :cvar T: Super-low rate Specifies that tax is applied at a rate
        ...
        (Lower rate). For use in ONIX 3.0 only
    :cvar Z: Zero-rated
    """
    H = "H"
    P = "P"
...
class List65(Enum):
    ...
    :cvar VALUE_09: Not yet available, postponed indefinitely Not yet
        ...
        included in &lt;SupplyDate&gt;. For use in ONIX 3.0 only
    ...
    :cvar VALUE_44: Apply direct Not available to trade, apply direct to
        publisher
    :cvar VALUE_45: Not sold separately Individual copies of the product
        are not available from the supplier, but packs of copies are
        available, and individual copies of the product may typically be
        sold at retail. Must be bought as part of a set or trade pack
...
class List68(Enum):
    ...
    :cvar VALUE_05: No longer our product Responsibility for the product
        ...
        PR.25 (ONIX 2.1) OR P.25 (ONIX 3.0))
    ...
    :cvar VALUE_13: Active, but not sold separately The product is
        published in this market and active but, as a publishing
        decision, it is not sold separately – only in an assembly or as
        part of a package. Depending on product composition and pricing,
        it may be saleable separately at retail
    :cvar VALUE_14: Active, with market restrictions The product is
        ...
        restriction. In ONIX 3.0, the &lt;SalesRestriction&gt; composite
        in Group P.24 should be used
    ...
    :cvar VALUE_16: Temporarily withdrawn from sale Temporarily
        ...
        technical reasons. In ONIX 3.0, must be accompanied by expected
        availability date coded ‘22’ within the &lt;MarketDate&gt;
        composite, except in exceptional circumstances where no date is
        known
    :cvar VALUE_17: Permanently withdrawn from sale Withdrawn
        permanently from sale in this market. Effectively synonymous
        with ‘Out of print’ (code 07), but specific to downloadable and
        online digital products (where no ‘stock’ would remain in the
        supply chain). For use in ONIX 3.0 only
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
    VALUE_02 = "02"
...
class List71(Enum):
    ...
    :cvar VALUE_13: To subscription services only Sales rights (or
        market distribution rights) apply to supplies to organisations
    ...
    :cvar VALUE_17: Through Inventoryless POD POD copies may be
        ...
        inventoryless). Only valid in ONIX 3.0
    :cvar VALUE_18: Through Stock Protection POD POD copies may be
        ...
        supplier. Only valid in ONIX 3.0
    :cvar VALUE_19: Except through POD Not eligible for POD. Only valid
        in ONIX 3.0
    :cvar VALUE_99: No restrictions on sales Positive indication that no
        ...
        subscription services and non-subscription customers. For use in
        ONIX 3.0 only
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
...
class List73(Enum):
    ...
    :cvar VALUE_41: Social networking URL for specific work or product
        For example, a Facebook, Google+, Instagram, Youtube, Pinterest,
        Tiktok, Twitter or similar URL for the product or work
    :cvar VALUE_42: Author’s social networking URL For example, a
        Facebook, Google+, Instagram, Youtube, Pinterest, Tiktok,
        Twitter or similar page
    ...
    :cvar VALUE_44: Social networking URL for specific article, chapter
        or content item For example, a Facebook, Google+, Instagram,
        Youtube, Pinterest, Tiktok, Twitter or similar page. Use only in
        the context of a specific content item (eg within
        &lt;ContentItem&gt;)
    ...
    :cvar VALUE_47: Publisher’s website for digital preservation The URL
        ...
        for access to its preserved copy. For use in ONIX 3.0 only
    :cvar VALUE_48: Third-party website for digital preservation The URL
        ...
        specific URL for access to its preserved copy. For use in ONIX
        3.0 only
    :cvar VALUE_49: Product website for environmental responsibility
        ...
        to the specific product. For use in ONIX 3.0 only
    :cvar VALUE_50: Organisation’s website for environmental
        responsibility statement The URL of a web page describing the
        environmental and sustainability policies, carbon neutrality
        status, etc of the organisation (publisher, supplier etc). For
        use in ONIX 3.0 only
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
...
class List74(Enum):
    ...
    :cvar EGL: Emilian ONIX local code for Italian dialect, equivalent
        to egl in ISO 639-3. For use in ONIX 3.0 only
    ...
    :cvar GRT: Garo ONIX local code, equivalent to grt in ISO 639-3. For
        use in ONIX 3.0 only
    ...
    :cvar MWF: Murrinh-Patha ONIX local code, equivalent to mwf in ISO
        639-3. For use in ONIX 3.0 only
    :cvar MWL: Mirandese
    ...
    :cvar NRF: Guernésiais, Jèrriais ONIX local code, equivalent to nrf
        in ISO 639-3. For use in ONIX 3.0 only
    ...
    :cvar PER: Persian; Farsi Macrolanguage
    ...
    :cvar PMS: Piedmontese ONIX local code for Italian dialect,
        equivalent to pms in ISO 639-3. For use in ONIX 3.0 only
    :cvar POL: Polish
    :cvar PON: Ponapeian
    :cvar POR: Portuguese
    ...
    :cvar QGL: Gallo ONIX local code, distinct variant of langue d’oïl
        ...
        fre, nrf by ISO 639-3). For use in ONIX 3.0 only
    :cvar QLK: Lemko ONIX local code, distinct dialect of of Rusyn (not
        distinguished from rue by ISO 639-3). For use in ONIX 3.0 only
    :cvar QLS: Neutral Latin American Spanish ONIX local code, distinct
        ...
        broad use across Latin America. For use in ONIX 3.0 only
    ...
    :cvar RGN: Romagnol ONIX local code for Italian dialect, equivalent
        to rgl in ISO 639-3. For use in ONIX 3.0 only
    ...
    :cvar SDC: Sassarese ONIX local code for Sardinian dialect,
        equivalent to sdc in ISO 639-3. For use in ONIX 3.0 only
    :cvar SDN: Gallurese ONIX local code for Sardinian dialect,
        equivalent to sdn in ISO 639-3. For use in ONIX 3.0 only
    :cvar SEL: Selkup
    ...
    :cvar SHI: Tashelhit; Shilha ONIX local code, equivalent to shi in
        ISO 639-3. For use in ONIX 3.0 only
    :cvar SHN: Shan
    :cvar SID: Sidamo
    ...
    :cvar SRM: Saramaccan ONIX local code for Saramaccan language,
        equivalent to srm in ISO 639-3. For use in ONIX 3.0 only
    :cvar SRN: Sranan Tongo
    :cvar SRO: Campidanese ONIX local code for Sardinian dialect,
        equivalent to sro in ISO 639-3. For use in ONIX 3.0 only
    :cvar SRP: Serbian
    :cvar SRR: Serer
    ...
    :cvar TZM: Central Atlas Tamazight ONIX local code, equivalent to
        tzm in ISO 639-3. For use in ONIX 3.0 only
    ...
    :cvar VEC: Venetian/Venetan ONIX local code for Italian dialect,
        equivalent to vec in ISO 639-3. For use in ONIX 3.0 only
    :cvar VEN: Venda
    :cvar VIE: Vietnamese
...
class List79(Enum):
    ...
    :cvar VALUE_25: Personal data requirements Personal data required
        ...
        described in &lt;ProductFormFeatureDescription&gt;. For use in
        ONIX 3.0 only
    ...
    :cvar VALUE_34: PEFC certified Product carries PEFC logo
        ...
        Custody (COC) number printed on the book. The Product Form
        Feature Description element may carry free text indicating the
        grade or type of paper. May be accompanied by a Post-Consumer
        Waste (PCW) percentage value, to be reported in another instance
    ...
    :cvar VALUE_38: ‘Green’ inks Vegetable-based or other
        ...
        more detailed statement. For use in ONIX 3.0 only
...
class List81(Enum):
    ...
    :cvar VALUE_10: Text (eye-readable) Readable text of the main work:
        ...
        designate an e-book or other digital or physical product whose
        primary content is eye-readable text
    :cvar VALUE_15: Extensive links between internal content
        E-publication contains a significant number of actionable cross-
        references, hyperlinked notes and annotations, or with other
        actionable links between largely textual elements (eg quiz/test
        questions, ‘choose your own ending’ etc)
    ...
    :cvar VALUE_16: Additional eye-readable text not part of main work
        Publication contains additional textual content such as
        interview, feature article, essay, bibliography, quiz/test,
    ...
    :cvar VALUE_41: Additional eye-readable links to external content
        ...
        URLs, QR codes etc). For use in ONIX 3.0 only
    ...
    :cvar VALUE_42: Assessment material eg Questions or student
        ...
        the work). For use in ONIX 3.0 only
    ...
    :cvar VALUE_34: Blank pages or spaces Entire pages or blank spaces,
        forms, boxes etc, intended to be filled in by the reader
    ...
    :cvar VALUE_40: Scripting E-publication contains microprograms
        ...
        system. For use in ONIX 3.0 only
    :cvar VALUE_43: Scripted pop-ups E-publication contains pop-ups or
        ...
        dictionary funcionality that is part of the reading system.] For
        use in ONIX 3.0 only
    """
    VALUE_10 = "10"
    VALUE_15 = "15"
...
class List9(Enum):
    ...
    :cvar VALUE_01: WCO Harmonized System World Customs Organization
        Harmonized Commodity Coding and Description System. Use 6 (or
        occasionally 8 or 10) digits, without punctuation
    :cvar VALUE_02: UNSPSC UN Standard Product and Service
        Classification. Use 8 (or occasionally 10) digits, without
        punctuation
    ...
    :cvar VALUE_05: TARIC EU TARIC codes, an extended version of the
        Harmonized System. Use 10 digits, without punctuation
    ...
    :cvar VALUE_13: HTSUS US HTS (or HTSA) commodity codes for import of
        goods into USA (10 digits, without punctuation). For use in ONIX
        3.0 only. See https://hts.usitc.gov/current
    :cvar VALUE_14: US Schedule B US Schedule B commodity codes for
        export from USA (10 digits, without punctuation). For use in
        ONIX 3.0 only. See http://uscensus.prod.3ceonline.com
...
class List92(Enum):
    ...
    :cvar VALUE_16: ISNI International Standard Name Identifier (used
        here to identify an organisation). For use in ONIX 3.0 only. See
        https://isni.org/
    :cvar VALUE_23: VAT Identity Number Identifier for a business
        organization for VAT purposes, eg within the EU’s VIES system.
        See http://ec.europa.eu/taxation_customs/vies/faqvies.do for EU
...
class List93(Enum):
    ...
    :cvar VALUE_13: Exclusive distributor to retailers and end-customers
        Intermediary as exclusive distributor to retailers and direct to
        consumers and/or institutional customers. For use in ONIX 3.0
        only
    :cvar VALUE_14: Non-exclusive distributor to retailers and end-
        customers Intermediary as non-exclusive distributor to retailers
        and direct to consumers and/or institutional customers. For use
        in ONIX 3.0 only
    :cvar VALUE_15: Distributor to retailers and end-customers Use only
        where exclusive/non-exclusive status is not known. Prefer codes
        13 or 14 as appropriate whenever possible. For use in ONIX 3.0
        only
    """
    VALUE_00 = "00"
    VALUE_01 = "01"
...
class List96(Enum):
    ...
    :cvar SKK: Slovak Koruna Slovakia. Now replaced by the Euro (EUR).
        Deprecated – use only for historical prices that pre-date the
        introduction of the Euro
    :cvar SLE: Leone Sierra Leone (from April 2022)
    :cvar SLL: Leone Sierra Leone (prices normally quoted as integers).
        Deprecated – replaced by SLE from April 2022 (SLE is a
        redenominatino of the Leone by a factor of 1,000)
    ...
    :cvar STN: Dobra São Tome and Principe. Replaced STD (old) Dobra at
        rate of 1000:1 in June 2018. For use in ONIX 3.0 only
    ...
    :cvar VES: Bolívar Soberano Venezuela (replaced VEF from August 2018
        ...
        of 1,000,000:1 in late 2021). For use in ONIX 3.0 only
